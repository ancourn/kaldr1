...
run_tests() {
    ...
    if command_exists cargo; then
        cargo test || echo "⚠️  Rust tests failed, but continuing..."
    else
    ...
    if command_exists npm; then
        npm test || echo "⚠️  TypeScript tests failed, but continuing..."
    else
        echo "⚠️  npm not found, skipping TypeScript tests"
    fi
}

# Function to build SDKs
build_sdks() {
    echo "🔨 Building SDKs..."
    ...
    cd "$PROJECT_ROOT/rust-client"
    if command_exists cargo; then
        cargo build --release || echo "⚠️  Rust build failed, but continuing..."
    else
    ...
    cd "$PROJECT_ROOT/typescript-client"
    if command_exists npm; then
        npm run build || echo "⚠️  TypeScript build failed, but continuing..."
    else
        echo "⚠️  npm not found, skipping TypeScript build"
    fi
}

# Main execution
echo "📋 SDK Generation Options:"
echo "1. Generate Rust SDK only"
echo "2. Generate TypeScript SDK only"
echo "3. Generate both SDKs"
echo "4. Generate and test both SDKs"
echo "5. Generate, test, and build both SDKs"
echo "6. Exit"
